...
| 五篇大文章各子场景的模板、映射版本、案例、Stage B 结果、历史、前端 session 和导出必须按 `scenario_id` 隔离；一个场景未命中或失败不得读取其他场景兜底 | 映射查询：`backend/tests/test_technology_finance_mapping_query.py`（四场景命中、相同 code 跨场景不得命中或兜底）✅；结果/历史/session/导出仍待 task 4.4、5.1、6.2 补齐 | 新增/开放五篇子场景，修改映射查询、结果保存、历史、前端 session 或导出 | ⏳分段建设（映射查询已建） |

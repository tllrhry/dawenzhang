...
import {
  ...
  consistencyLabels,
  inclusiveBorrowerBasis,
  inclusiveCreditSummary,
  stageBEmptyDescription,
  stageBStatusLabel,
  stageBStatusLabels,
  statusColor,
} from './components/FinanceResultCards'
import { caseMatchesScenario, currentCaseStorageKey, DIGITAL_FINANCE_SCENARIO, fiveArticlesScenarioIds, GREEN_FINANCE_SCENARIO, INCLUSIVE_FINANCE_SCENARIO, isFiveArticlesScenario, PENSION_FINANCE_SCENARIO, scenarioViews, TECHNOLOGY_FINANCE_SCENARIO } from './scenarios'
...
function pensionHistorySummary(scenarioId: ScenarioId, result: FiveArticlesResult): string | null {
  if (scenarioId !== PENSION_FINANCE_SCENARIO) return null
...
function greenHistorySummary(scenarioId: ScenarioId, result: FiveArticlesResult): string | null {
  if (scenarioId !== GREEN_FINANCE_SCENARIO) return null
  const direction = result.consistency_evidence_refs.find((reference) => reference.type === 'green_direction')
  const warnings = result.consistency_evidence_refs
    .filter((reference) => (reference.type === 'green_auxiliary' || reference.type === 'green_violation') && reference.warning)
    .map((reference) => reference.warning)
  const directory = [direction?.subject, ...(direction?.taxonomy_path || [])].filter(Boolean).join(' / ')
  const method = direction?.match_method === 'condition_fallback' ? '条件回退命中' : '行业编码命中'
  return `${directory || '绿色目录待确认'} · ${method}${warnings.length > 0 ? ` · ${warnings.join('；')}` : ' · 辅助证据完整'}`
}

function technologyHistorySummary(scenarioId: ScenarioId, result: FiveArticlesResult): string | null {
  if (scenarioId !== TECHNOLOGY_FINANCE_SCENARIO) return null
  const direction = result.consistency_evidence_refs.find((reference) => reference.type === 'technology_direction')
  const warnings = result.consistency_evidence_refs
    .filter((reference) => reference.type === 'technology_auxiliary' && reference.warning)
    .map((reference) => reference.warning)
  const directionSummary = direction?.mapping_hit
    ? `科技投向：${direction.NEIC_Code || '--'} ${direction.NEIC_Name || ''}`
    : '科技投向未命中'
  return `${directionSummary}${warnings.length > 0 ? ` · ${warnings.join('；')}` : ' · 辅助证据完整'}`
}
  ...
  return <main className="page-content history-page">
    ...
    <Card className="history-card" bordered={false}>
      ...
      {history.map((item) => isFiveArticlesResult(item)
        ? <div className="history-row" key={`stage-b-${item.id}`}><b>{caseData?.original_filename || '当前企业案例'}<small>{scenarioView.name}版本 {item.version} · Stage A #{item.stage_a_result_id}</small></b><span className="history-conclusion"><span><strong>{item.loan_neic_code || '--'}</strong> {item.loan_neic_name || '暂无正式标签'}</span><small>{item.labels.length} 个{scenarioView.name}标签{item.consistency_status ? ` · ${consistencyLabels[item.consistency_status]}` : ''}</small></span><span className="history-basis">{stageBEmptyDescription(scenarioId, item)}{technologyHistorySummary(scenarioId, item) && <small>{technologyHistorySummary(scenarioId, item)}</small>}{pensionHistorySummary(scenarioId, item) && <small>{pensionHistorySummary(scenarioId, item)}</small>}{digitalHistorySummary(scenarioId, item) && <small>{digitalHistorySummary(scenarioId, item)}</small>}{greenHistorySummary(scenarioId, item) && <small>{greenHistorySummary(scenarioId, item)}</small>}</span><span><Tag color={statusColor(item.status)}>{stageBStatusLabel(scenarioId, item.status)}</Tag></span><span>{new Date(item.created_at).toLocaleString('zh-CN')}</span><button type="button" onClick={() => navigate(scenarioView.classifyPath)}>查看详情 <ArrowRightOutlined /></button></div>
        ...
        : isInclusiveFinanceResult(item)
          ? <div className="history-row" key={`inclusive-${item.id}`}><b>{caseData?.original_filename || '当前企业案例'}<small>普惠版本 {item.version} · Stage A #{item.stage_a_result_id}</small></b><span className="history-conclusion"><strong>{item.inclusive_category || '未形成普惠子类别'}</strong><small>计算规模：{item.computed_size || '待复核'}</small><small>填写规模：{item.filled_size || '未填写或不适用'}</small></span><span className="history-basis"><span>{inclusiveCreditSummary(item)}</span><small>{inclusiveBorrowerBasis(item)}</small><small>{item.basis || item.error_detail || '暂无说明'}</small></span><span><Tag color={statusColor(item.status)}>{item.status === 'not_applicable' ? '不属于普惠金融' : stageBStatusLabels[item.status]}</Tag></span><span>{new Date(item.created_at).toLocaleString('zh-CN')}</span><button type="button" onClick={() => navigate(scenarioView.classifyPath)}>查看详情 <ArrowRightOutlined /></button></div>

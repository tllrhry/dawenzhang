...
import { currentCaseStorageKey, INCLUSIVE_FINANCE_SCENARIO, scenarioViews } from './scenarios'
...
function FiveArticlesScenario({ onEnter }: { onEnter: () => void }) {
  return (
    <Card className="scenario-card scenario-card-active" bordered={false}>
      ...
      <div className="scene-body">
        <h2>五篇大文章分类</h2>
        <p>科技金融与普惠金融已开放，分别提供映射判定和确定性普惠判定。</p>
        <Tag className="available-tag" icon={<CheckCircleFilled />}>科技金融、普惠金融已开放</Tag>
        <Button type="primary" size="large" block icon={<ArrowRightOutlined />} iconPosition="end" onClick={onEnter}>
          进入科技金融
        </Button>
        <Button block onClick={() => window.location.assign(scenarioViews[INCLUSIVE_FINANCE_SCENARIO].classifyPath)}>进入普惠金融</Button>
        <div className="topic-tags"><span>绿色金融 · 暂未开放</span><span>养老金融 · 暂未开放</span><span>数字金融 · 暂未开放</span></div>
      </div>
      <Divider />
...
function ClassifyPage({ scenarioId }: { scenarioId: ScenarioId }) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      scenarioId !== NATIONAL_ECONOMY_SCENARIO ? getHistory(scenarioId, caseId) : Promise.resolve([]),
  ...
  return <section className="result-layout">
    ...
    {scenarioId === INCLUSIVE_FINANCE_SCENARIO && <Card className="result-card technology-result-card" bordered={false} title="Stage B · 普惠金融判定">
      {stageBResult && isInclusiveFinanceResult(stageBResult) ? <Descriptions column={1} size="small">
        <Descriptions.Item label="判定状态"><Tag color={statusColor(stageBResult.status)}>{stageBStatusLabels[stageBResult.status]}</Tag></Descriptions.Item>
